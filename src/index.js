document.addEventListener("DOMContentLoaded", () => { // Here we use DOMContentLoaded to ensure the HTML is fully loaded before running the script
  /************  HTML ELEMENTS  ************/
  // View divs
  ...
  const questions = [ // Create new Question objects and add them to the questions array
    new Question("What is 2 + 2?", ["3", "4", "5", "6"], "4", 1), // Question and different choices
    new Question("What is the capital of France?", ["Miami", "Paris", "Oslo", "Rome"], "Paris", 1), // Question and different choices
    new Question("Who created JavaScript?", ["Plato", "Brendan Eich", "Lea Verou", "Bill Gates"], "Brendan Eich", 2), // Question and different choices
    new Question("What is the mass–energy equivalence equation?", ["E = mc^2", "E = m*c^2", "E = m*c^3", "E = m*c"], "E = mc^2", 3),  //Question and diferent choices
  ...
  const timeRemainingContainer = document.getElementById("timeRemaining"); // The time remaining container is the element that SHOWS the time left in the quiz
  timeRemainingContainer.innerText = `${minutes}:${seconds}`; // Muestra el tiempo restante en formato MM:SS
  ...
  function showQuestion() { // Displays the current question and its choices
  ...
  function nextButtonHandler () { // Handles the click on the next button 
